...
def main():
    ...
    # User inputs
    simulation_start_date = st.date_input("Simulation Start Date", value=start_date)
    simulation_end_date = st.date_input("Simulation End Date", value=end_date)
    ...
    # Convert simulation start and end dates to Timestamp
    simulation_start_date = pd.Timestamp(simulation_start_date)
    simulation_end_date = pd.Timestamp(simulation_end_date)

    # Calculate equity curves
    benchmark_equity_curve, _ = calculate_benchmark_equity_curve(simulation_start_date, simulation_end_date, initial_equity)
    ...
    benchmark_cagr, benchmark_volatility, benchmark_sharpe_ratio = calculate_performance_metrics(benchmark_equity_curve)
    sample_strategy_cagr, sample_strategy_volatility, sample_strategy_sharpe_ratio = calculate_performance_metrics(
        sample_strategy_equity_curve)
...
def calculate_benchmark_equity_curve(start_date, end_date, initial_equity):
    ...
    benchmark_prices = close_prices.loc[start_date:end_date]

    # Calculate the number of shares for each stock
    allocation_per_stock = initial_equity / len(nifty50_symbols)
    shares_per_stock = allocation_per_stock / benchmark_prices.iloc[0]

    # Calculate the portfolio value for each day
    benchmark_equity_curve = (benchmark_prices * shares_per_stock).sum(axis=1)

    return benchmark_equity_curve, shares_per_stock
